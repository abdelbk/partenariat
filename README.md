...
Simplébo a développé un système de partenariat qui s'appuie sur des "codes partenaires" fournis par Simplébo afin d'identifier le partenaire et l'offre auquel pourrait souscrire le client.

Ce code peut être utilisé par le client de plusieurs façons.
...
Simplébo fournit à devispresto un formulaire permettant à son client de remplir ses informations de base afin de se faire rappeler. Ce formulaire peut être intégré au site devispresto via une iframe ou être atteint par le client via un simple lien.
...
## Checkbox "je suis intéressé" dans le panier d'achat

### Intégration de la checkbox au panier
La checkbox doit être intégrée au panier par devispresto à côté d'une phrase du type :
```
...
La selection de la checkbox doit entraîner l'envoi d'une requête du serveur devispresto au serveur Simplébo afin de transmettre les données clients.
...
// Vars initialization
$partner_api_url = 'https://simplebo.fr/partner-api'
